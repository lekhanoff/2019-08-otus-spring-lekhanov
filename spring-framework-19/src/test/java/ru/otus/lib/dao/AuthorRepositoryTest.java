...
public class AuthorRepositoryTest {
    ...
    @DisplayName("Search authors by params")
    @Test
    void testGetByParams() {
        List<Author> authorList = authorDao.findByParams("leo");
        assertThat(authorList).hasSize(1).containsExactly(authorLeoTolstoy);
        
        authorList = authorDao.findByParams("TOLS");
        assertThat(authorList).hasSize(1).containsExactly(authorLeoTolstoy);
        
        authorList = authorDao.findByParams("ich");
        assertThat(authorList).hasSize(2).containsExactly(authorLeoTolstoy, authorFedorDostoevsky);
    }
}

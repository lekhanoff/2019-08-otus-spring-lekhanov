...
public class BookCommentServiceTest {
    ...
    void testCreateBookComment() {
        ...
        Mockito.when(bookCommentRepo.findByBookBookIdAndUserLogin(Long.valueOf(bookId), userName)).thenReturn(Optional.empty());
    ...
    void testUpdateBookComment() {
        ...
        Mockito.when(bookService.getById(Long.valueOf(bookId))).thenReturn(Optional.of(warAndPeace));
        Mockito.when(bookCommentRepo.existsById(testComment1.getBookCommentId())).thenReturn(true);
        Mockito.when(bookCommentRepo.findByBookBookIdAndUserLogin(Long.valueOf(bookId), userName)).thenReturn(Optional.of(testComment1));
    ...
    void testGetBookCommentsByBookId() {
        ...
        Mockito.when(messageSource.getMessage("bookComment.enter.book-id", null, Locale.getDefault())).thenReturn(enterBookIdMessage);
        Mockito.when(lineReader.readLine(enterBookIdMessage)).thenReturn(bookId);
        Mockito.when(bookCommentRepo.findByBookBookId(Long.valueOf(bookId))).thenReturn(Arrays.asList(testComment1, testComment2));

...
public class GenreServiceTest {
    ...
    public void init() throws IOException {
        genreService = new GenreServiceImpl(genreDao, bookService, messageSource, lineReader);
    }
    ...
    void testCreateGenre() {
        ...
        Mockito.when(genreDao.createGenre(Genre.builder().name(genreName).build())).thenReturn(russianClassic);
        ...
        assertEquals(1, Mockito.mockingDetails(genreDao).getInvocations().size());
    ...
    void testUpdateGenre() {
        ...
        Mockito.when(genreDao.getById(genreId)).thenReturn(Optional.of(fantasy));
        Mockito.when(genreDao.updateGenre(genreId, Genre.builder().name(genreName).build())).thenReturn(fantasy);
        ...
        assertEquals(2, Mockito.mockingDetails(genreDao).getInvocations().size());
    }
    ...
    void testGetById() {
        ...
        Mockito.when(genreDao.getById(1000L)).thenReturn(Optional.of(russianClassic));
    ...
    void testGetByName() {
        ...
        Mockito.when(genreDao.getByName("Russian Classic")).thenReturn(Optional.of(russianClassic));
    ...
    @Test
    void testGetAll() {
        Mockito.when(genreDao.getAll()).thenReturn(Arrays.asList(russianClassic, foreignClassic));

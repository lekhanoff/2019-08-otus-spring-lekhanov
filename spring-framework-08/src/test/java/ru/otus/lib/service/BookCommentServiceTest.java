...
public class BookCommentServiceTest {
    ...
    private Genre russianClassic = Genre.builder().id("100").name("Русская классика").build();
    ...
    void testCreateBookComment() {
        ...
        Mockito.when(bookService.getById(bookId)).thenReturn(Optional.of(warAndPeace));
        Mockito.when(bookCommentRepo.findByBookIdAndUserLogin(bookId, userName)).thenReturn(Optional.empty());
        Mockito.when(bookCommentRepo.save(BookComment.builder().book(warAndPeace).userLogin(userName).userComment(userComment).build())).thenReturn(testComment1);
    ...
    void testUpdateBookComment() {
        ...
        String bookCommentId = testComment1.getId();
        String bookId = "1000";
        String userName = "ivanov";
        String userComment = "Super book";
        ...
        Mockito.when(bookService.getById(bookId)).thenReturn(Optional.of(warAndPeace));
        Mockito.when(bookCommentRepo.existsById(testComment1.getId())).thenReturn(true);
        Mockito.when(bookCommentRepo.findByBookIdAndUserLogin(bookId, userName)).thenReturn(Optional.of(testComment1));
        Mockito.when(bookCommentRepo.save(testComment1)).thenReturn(testComment1);
    ...
    void testDeleteBookComment() {
        ...
        String bookCommentId = testComment2.getId();
    ...
    void testGetById() {
        ...
        Mockito.when(bookCommentRepo.findById(bookCommentId)).thenReturn(Optional.of(testComment1));
    ...
    void testGetBookCommentsByBookId() {
        String enterBookIdMessage = "Enter book id:"; 
        String bookId = warAndPeace.getId();
        
        Mockito.when(messageSource.getMessage("bookComment.enter.book-id", null, Locale.getDefault())).thenReturn(enterBookIdMessage);
        Mockito.when(lineReader.readLine(enterBookIdMessage)).thenReturn(bookId);
        Mockito.when(bookCommentRepo.findByBookId(bookId)).thenReturn(Arrays.asList(testComment1, testComment2));
